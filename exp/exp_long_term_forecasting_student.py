...
from exp.exp_basic_student import Exp_Basic
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_x_img, batch_y_img, batch_x_weather, batch_y_weather) in enumerate(vali_loader):
                ...
                outputs = self.model(batch_x, batch_x_img, batch_y_img, batch_x_weather, batch_y_weather)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_x_img, batch_y_img, batch_x_weather, batch_y_weather) in enumerate(train_loader):
                ...
                if self.loss_type == 'Weighted_MSE_MAE':
                    loss_mse = criterion(outputs, batch_y)
                    loss_mae = criterion_mae(outputs, batch_y)
                    loss = loss_mse * self.args.alpha_weight + loss_mae * (1 - self.args.alpha_weight)
                else:
                    loss = criterion(outputs, batch_y)
            ...
            vali_loss, vali_ts_loss = self.vali(vali_data, vali_loader, criterion)
            test_loss, test_ts_loss = self.vali(test_data, test_loader, criterion)
