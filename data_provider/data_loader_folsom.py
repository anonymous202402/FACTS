...
class SlidingWindowLogicalDailySolarDataset(Dataset):
    def __init__(self, root_path, data_path='Target_intra-hour.csv', flag='train',
        ...
        print(f"Logical day configuration: start {day_start_time_str}, end {day_end_time_str} (relative to start calendar day)")
        print(f"Expected points per logical day: {self.points_per_logical_day} (based on {self.minutes_interval}-minute intervals)")
        print(f"Input sequence points (seq_len): {self.seq_len}")
        print(f"Prediction sequence points (pred_len): {self.pred_len}")
        print(f"Sliding window stride (stride_points): {self.stride_points}")
