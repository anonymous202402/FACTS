...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross, return_intermediates=False):
        final_predict = None
        decoder_features = []  # 新增：存储解码器中间特征
        layer_predictions = []  # 新增：存储每层预测
        i = 0

        ts_d = x.shape[1]

        if return_intermediates:
            decoder_features.append(x.clone())  # 保存初始解码器输入
        ...
        for layer in self.decode_layers:
            ...
            if return_intermediates:
                decoder_features.append(x.clone())  # 保存每层解码器输出
                layer_predictions.append(layer_predict.clone())  # 保存每层预测
        ...
        if return_intermediates:
            return final_predict, {
                'decoder_features': decoder_features,  # 解码器每层的隐藏状态
                'layer_predictions': layer_predictions  # 每层的预测输出
            }
        else:
            return final_predict
