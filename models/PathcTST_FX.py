...
class ContinuousLoss(nn.Module):
    def __init__(self, cond_dim, width, depth, num_heads):
        super().__init__()

        self.cond_proj = nn.Linear(cond_dim, width)

        # 替换为标准 Transformer Encoder
        ...
        self.output_layer = nn.Linear(width, 1)  # 输出一个连续值

    def forward(self, x):

        x = self.cond_proj(x).unsqueeze(1)  # [B, 1, D]
        x = self.transformer(x)                # [B, 1, D]
        out = self.output_layer(x[:, 0])       # [B, 1]
        return out
